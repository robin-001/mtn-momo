...
class Disbursements extends MoMoClient
{
    /**
    ...
    public function transfer($amount, $currency, $externalId, $payeeId, $payeeIdType = 'MSISDN', $payerMessage = '', $payeeNote = '')
    {
        ...
        $response = $this->client->post("{$this->apiBaseUrl}/disbursement/v1_0/transfer", [
            'headers' => [
    ...
    public function getTransferStatus($referenceId)
    {
        ...
        $response = $this->client->get("{$this->apiBaseUrl}/disbursement/v1_0/transfer/{$referenceId}", [
            'headers' => [
                'Authorization' => "Bearer {$token}",
                'X-Target-Environment' => config('momo.environment', 'sandbox'),
                'Ocp-Apim-Subscription-Key' => $this->apiKey
    ...
    public function getAccountBalance()
    {
        ...
        $response = $this->client->get("{$this->apiBaseUrl}/disbursement/v1_0/account/balance", [
            'headers' => [
                'Authorization' => "Bearer {$token}",
                'X-Target-Environment' => config('momo.environment', 'sandbox'),
                'Ocp-Apim-Subscription-Key' => $this->apiKey
    ...
    public function checkAccountHolderStatus($accountHolderId, $accountHolderIdType = 'MSISDN')
    {
        ...
        $response = $this->client->get("{$this->apiBaseUrl}/disbursement/v1_0/accountholder/{$accountHolderIdType}/{$accountHolderId}/active", [
            'headers' => [
                'Authorization' => "Bearer {$token}",
                'X-Target-Environment' => config('momo.environment', 'sandbox'),
                'Ocp-Apim-Subscription-Key' => $this->apiKey
